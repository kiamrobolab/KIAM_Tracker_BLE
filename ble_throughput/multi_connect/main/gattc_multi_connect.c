...
#define CONN_DEVICES (int)(conn_device_a) + (int)(conn_device_b) + (int)(conn_device_c) + (int)(conn_device_d)
...
static const char remote_device_address[4][18] = {"30 ae a4 f4 17 6e",
		                                          "30 ae a4 f4 87 da",
												  "30 ae a4 fe 79 46",
												  "30 ae a4 f4 7c f2"};
...
static void hex_array_to_string(const void *hex_arr, const uint16_t len, char *string)
{
	//function doesn't check the volume of allocated for char pointer memory!
	if ( len == 0 ) return;
	char temp_buffer[(len+3)/4*4];   //for not-byte-accessible memory
	const char *ptr_line;

	if ( !esp_ptr_byte_accessible(hex_arr) ) {
	    //use memcpy to get around alignment issue
	    memcpy( temp_buffer, hex_arr, (len+3)/4*4 );
...
static void gattc_profile_a_event_handler(esp_gattc_cb_event_t event, esp_gatt_if_t gattc_if, esp_ble_gattc_cb_param_t *param)
{
    ...
    //ESP_LOGI(GATTC_TAG, "Event %d occured", (int)event);
...
static void gattc_profile_c_event_handler(esp_gattc_cb_event_t event, esp_gatt_if_t gattc_if, esp_ble_gattc_cb_param_t *param)
{
    ...
    case ESP_GATTC_CONNECT_EVT:{
        ...
        ESP_LOGI(GATTC_TAG, "REMOTE BDA:");
        esp_log_buffer_hex(GATTC_TAG, gl_profile_tab[PROFILE_C_APP_ID].remote_bda, sizeof(esp_bd_addr_t));
        esp_err_t mtu_ret = esp_ble_gattc_send_mtu_req (gattc_if, p_data->connect.conn_id);
        if (mtu_ret){
            ESP_LOGE(GATTC_TAG, "config MTU error, error code = %x", mtu_ret);
        }
...
static void gattc_profile_d_event_handler(esp_gattc_cb_event_t event, esp_gatt_if_t gattc_if, esp_ble_gattc_cb_param_t *param)
{
    ...
    case ESP_GATTC_CONNECT_EVT: {
    	ESP_LOGI(GATTC_TAG, "ESP_GATTC_CONNECT_EVT conn_id %d, if %d", p_data->connect.conn_id, gattc_if);
        gl_profile_tab[PROFILE_D_APP_ID].conn_id = p_data->connect.conn_id;
        memcpy(gl_profile_tab[PROFILE_D_APP_ID].remote_bda, p_data->connect.remote_bda, sizeof(esp_bd_addr_t));
        ESP_LOGI(GATTC_TAG, "REMOTE BDA:");
        esp_log_buffer_hex(GATTC_TAG, gl_profile_tab[PROFILE_D_APP_ID].remote_bda, sizeof(esp_bd_addr_t));
        esp_err_t mtu_ret = esp_ble_gattc_send_mtu_req (gattc_if, p_data->connect.conn_id);
        if (mtu_ret){
            ESP_LOGE(GATTC_TAG, "config MTU error, error code = %x", mtu_ret);
        }
        ...
        case ESP_GAP_SEARCH_INQ_RES_EVT:
            ...
            if (strncmp(bda, remote_device_address[0], address_name_len) == 0) {
                if (conn_device_a == false) {
                    ...
                    esp_ble_gap_stop_scanning();
                    esp_ble_gattc_open(gl_profile_tab[PROFILE_A_APP_ID].gattc_if, scan_result->scan_rst.bda, scan_result->scan_rst.ble_addr_type, true);
                    ESP_LOGI(GATTC_TAG, "GATTC open success");
                    Isconnecting = true;
                }
                break;
            }
            else if (strncmp(bda, remote_device_address[1], address_name_len) == 0) {
                if (conn_device_b == false) {
                    conn_device_b = true;
                    ESP_LOGI(GATTC_TAG, "Searched device %s", remote_device_name[1]);
                    esp_ble_gap_stop_scanning();
            ...
            else if (strncmp(bda, remote_device_address[2], address_name_len) == 0) {
                if (conn_device_c == false) {
                    conn_device_c = true;
                    ESP_LOGI(GATTC_TAG, "Searched device %s", remote_device_name[2]);
                    esp_ble_gap_stop_scanning();
            ...
            else if (strncmp(bda, remote_device_address[3], address_name_len) == 0) {
