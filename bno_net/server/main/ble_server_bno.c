...
void app_main()
{
    ...
    printf("bno055_set_default_conf() returned %02x \n", err);

    err = bno055_open(i2c_num, &bno_conf);
    printf("bno055_open() returned %02x \n", err);
    ...
    err = bno055_set_ext_crystal_use(i2c_num, 1 );
    printf("bno055_set_ext_crystal_use() returned %02x \n", err);
    vTaskDelay(30 / portTICK_RATE_MS);

    bno055_chip_info_t chip_inf;
    err = bno055_get_chip_info(i2c_num, &chip_inf);
    printf("bno055_get_chip_info() returned %02x \n", err);
    vTaskDelay(10 / portTICK_RATE_MS);

    if( err == ESP_OK) bno055_displ_chip_info(chip_inf);

    err = bno055_set_mode(i2c_num, OPERATION_MODE_NDOF);
    printf("bno055_set_mode() returned %02x \n", err);
    vTaskDelay(1000 / portTICK_RATE_MS);
